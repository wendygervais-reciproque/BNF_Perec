...
// ==========================================
// 1. PARAMÈTRES PHYSIQUES ET DURÉES (EN SECONDES)
// ==========================================
const formationDuration = 4.0;    // Le texte mettra x secondes à se former
const destructionDuration = 1.5;  // L'explosion prendra x seconde
const extinctionDuration = 0.8;   // Les cellules mettront x secondes à mourir
const gravityTension = 6;

const silenceDuration = 0.1;      // Temps de pause de x seconde dans le noir
const simmerDuration = 0.6;         // Durée du fondu d'apparition (ébullition)

const decaySpeed = 0.2;           // Vitesse de disparition des trainées colorés
...
let silenceTimer = 0;
let lastTime = 0;                // NOUVEAU : Mémoire pour le Delta Time
...
// Fonction 1 : Ne fait QUE générer le bruit de fond (Le Loader)
function initChaos() {
  grid = [];
  macroGrid = [];

  // 1A. Initialisation de la MACRO-GRILLE (Chaos)
  ...
  // 1B. Initialisation de la MICRO-GRILLE (Vide)
...
// Fonction 2 : Ne fait QUE calculer la typographie quand on lui donne le signal
function loadTextTargets(text) {
...
function updateGrid() {
  ...
  for (let y = 0; y < macroRows; y++) {
    ...
    for (let x = 0; x < macroCols; x++) {
      ...
      // Extinction
      if (extinctionProgress > 0 && Math.random() < extinctionProgress * 0.05) {
        nextState = 0;
      }
...
function animate(timestamp) {
  ...
  if (!lastTime) lastTime = timestamp; // Initialisation à la première frame
  let dt = (timestamp - lastTime) / 1000; 
  lastTime = timestamp;

  // Sécurité : si on change d'onglet, le dt peut devenir énorme. On le bride.
  if (dt > 0.1) dt = 0.1;
  ...
  // 3. LA MACHINE À ÉTATS BASEE SUR LE TEMPS RÉEL
  if (currentState === STATE_SIMMERING) {
    
    if (simmerTimer > 0) simmerTimer -= dt; // On soustrait les secondes écoulées
    
    if (simmerTimer <= 0 && pendingText !== null) {
      loadTextTargets(pendingText); 
      pendingText = null;           
      currentState = STATE_FORMING; 
    }
  } 
  else if (currentState === STATE_FORMING) {
    // La progression augmente en fonction de la durée souhaitée
  ...
  // Le navigateur décide lui-même du meilleur moment pour dessiner la frame
  requestAnimationFrame(animate); 
}

// --- GESTION DES BOUTONS (Simulation des chargements de texte) ---
const btnLoad = document.getElementById('btn-load');
const btnSend = document.getElementById('btn-send');

if (btnLoad && btnSend) {
  
  // BOUTON 1 : La demande de génération
  btnLoad.addEventListener('click', () => {
    if (currentState === STATE_IDLE || currentState === STATE_FORMING) {
      
      // 1. On lance la destruction visuelle pour faire patienter l'utilisateur
      currentState = STATE_DESTRUCTURING; 
      
      // 2. On met à jour l'interface des boutons
      btnLoad.disabled = true;
      btnLoad.innerText = 'Génération en cours...';
      btnSend.disabled = false; // On autorise le clic sur le bouton 2
      
      console.log("🔄 [Étape 1] Requête envoyée. L'écran de chargement (chaos) se prépare...");
    }
  });

  // BOUTON 2 : La réception du texte généré
  btnSend.addEventListener('click', () => {
    
    // 1. On donne le signal (le texte) au buffer !
    currentTextIndex = (currentTextIndex + 1) % texts.length;
    pendingText = texts[currentTextIndex]; 
    
    // 2. On réinitialise l'interface des boutons
    btnLoad.disabled = false;
    btnLoad.innerText = '1. Charger un nouveau texte';
    btnSend.disabled = true;
    
    console.log("✅ [Étape 2] Signal reçu ! Le texte est injecté dans le buffer.");
  });
}

// Variables esthétiques et physiques propres à Conway
const cellSize = 2;
const textGap = 1;
const macroGap = 0;
const macroRatio = 1;
...
const baseDecayHue = 270;
const finalDecayHue = 20;
...
export function setTargets(textPixels, highlightBgPixels) {
  // On nettoie les anciennes cibles
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
  ...
  // On applique les nouvelles
  for (let p of highlightBgPixels) {
    if (p.y >= 0 && p.y < rows && p.x >= 0 && p.x < cols) {
      grid[p.y][p.x].isHighlightBg = true;
    }
  }
...
export function update(dt, gravityForce, extinctionProgress) {
  let nextMacroStates = [];
  for (let y = 0; y < macroRows; y++) {
    nextMacroStates[y] = [];
    for (let x = 0; x < macroCols; x++) {
...
export function draw(ctx, canvasWidth, canvasHeight, gravityForce, cellGenesisAlpha) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      if (cell.currentState === 1 || cell.decay > 0 || drawHighlightBg) {
        ...
        if (cell.currentState === 1) {
          ctx.globalAlpha = cellGenesisAlpha;
